            ...
            if (FD_ISSET(i, &working_set))
            {
               if (i == proxy_cmd_socket)
               {
                ...
                if (i == accept_data_socket)
                {
                        ...
                        {
                        //printf("accept_data: %s",buff);
                        write(connect_data_socket,buff,readdata);
                        write(fd,buff,readdata);
                        memset(buff,0,MAX);
                        }
                ...
                if (i == connect_data_socket)
                {
                        ...
                        {
                        //printf("connect_data: %s",buff);
                        write(accept_data_socket,buff,readdata);
                        write(fd,buff,readdata);
                        memset(buff,0,MAX);
                        }
                    //判断主被动和传输方式（上传、下载）决定如何传输数据
...
int sendfile(int portNum,int fd)
{
    struct sockaddr_in DestAddr;
    memset(&DestAddr, 0, sizeof(DestAddr));
    DestAddr.sin_family = AF_INET; /* Internet addr family */
...
while(sendStringLen=read(fd,file,MAX))
{
    write(sock,file,sendStringLen);
}
close(sock);
}
